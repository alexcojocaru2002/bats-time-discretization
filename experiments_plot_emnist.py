...
def load_data(experiments_path, dt_values):
    ...
    for exp_folder in experiment_folders:
        ...
        for dt in dt_values:
            ...
            file_path_prediction = os.path.join(full_exp_path, f'Output Spikes DT = {dt}')
...
# New function to plot accuracy over test data against total spike count with reversed axis, Pareto frontier, and error bars
def plot_accuracy_vs_spike_count(accuracy_stats, hidden_spike_stats, output_spike_stats, ylabel, title):
    ...
    for dt in accuracy_stats.keys():
        ...
        std_hidden_spikes = accuracy_stats[dt]['std']
        std_output_spikes = accuracy_stats[dt]['std']
        total_spikes = 1/(avg_hidden_spikes + avg_output_spikes)
        total_spikes_std = np.sqrt(std_hidden_spikes**2 + std_output_spikes**2)

        plt.scatter(total_spikes, avg_accuracy, marker='o', s=50, label=f'DT = {dt}')
        plt.errorbar(total_spikes, avg_accuracy, yerr=total_spikes_std, fmt='o', capsize=5)

        # Compute Pareto frontier points
        points = sorted(zip(total_spikes, avg_accuracy), reverse=True)
        pareto_front = []
        max_acc = -np.inf
    ...
    all_pareto_points.sort(reverse=True)
    ...
    plt.plot(spikes, accs, linestyle='-', linewidth=4, color='red', label='Pareto Frontier')
    plt.scatter(spikes, accs, s=100, marker='x', color='red')
...
# Main execution
experiments_path = 'experiments/emnist/output_metrics/'  # Change this to your actual path
dt_values = ['0.0', '0.0001', '0.0005', '0.001', '0.002', '0.004', '0.005', '0.007', '0.008']
latency_values = [0.02, 0.03, 0.05, 0.08, 0.1, 0.13, 0.15, 0.18, 0.2]

prediction_confidence, accuracy_data_train, accuracy_data_test, hidden_spike_data, output_spike_data, time_data_train = load_data(
...
plot_statistics(hidden_spike_stats, 'Hidden Layer 1 Spike Counts',
                'Average and Standard Deviation of Hidden Layer 1 Spike Counts per Epoch for Different DT Values')
